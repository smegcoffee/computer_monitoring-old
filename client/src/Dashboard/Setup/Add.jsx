...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, Checkbox, TextField, Autocomplete } from '@mui/material';
...
const SearchableDropdown = ({ options, placeholder, onSelect }) => {
    ...
    const handleCheckboxClick = (index) => {
      if(checkedRows.includes(index)){
        setCheckedRows(checkedRows.filter((item) => index));
      } else {
  ...
  return (
    <div className="z-10 fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
      <div className="bg-white rounded-tl-xl rounded-tr-xl rounded-bl-xl rounded-br-xl shadow-md" style={{minWidth: '1100px', maxWidth:'100vh', maxHeight:'100vh'}}>
          ...
          <div className='flex-none'>
            <TableContainer component={Paper} style={{borderTopLeftRadius: '10px', borderTopRightRadius: '10px'}}>
              <Table>
                <TableHead>
                  <TableRow className='bg-red-200'>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>UNIT CODE</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>DATE OF PURCHASE</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>CATEGORY</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>DESCRIPTION</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>SUPPLIER</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>SERIAL NO.</Typography></TableCell>
                    <TableCell align='center'><Typography variant='subtitle1' style={{fontWeight: 700}}>STATUS</Typography></TableCell>
                ...
                <TableBody>
                  {filteredData.slice(page * rowsPerPage, (page + 1) * rowsPerPage).map((data, index) => (
                  <TableRow key={index}>
                    <TableCell align='center'>{data.unit}</TableCell>
                    <TableCell align='center'>{data.dop}</TableCell>
                    <TableCell align='center'>{data.category}</TableCell>
        ...
        <div className='flex-1 justify-center items-center text-center'>
            ...
            <button type='submit' className='bg-green-600 h-8 w-24 text-white rounded-full ml-3 text-sm font-semibold'>ADD</button>
        </div>
        </div>
        </form>
        </div>
